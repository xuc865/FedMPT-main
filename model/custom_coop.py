...
class CoOpPromptLearner(nn.Module):
    def __init__(self, cfg):
        ...
        self.vas = self.ctx.detach().clone()
...
class CoOpCLIP(nn.Module):
    ...
    def forward(self, image,classnames=None, dataname=None):
    ...
    def encode_text(self,classnames):
        ...
        ctx, vas = self.prompt_learner()
        ctx = ctx.to(self.dtype)
        vas = vas.to(self.dtype)
        ctx = ctx.unsqueeze(0).expand(token_prefix.shape[0], -1, -1)
        vas = vas.unsqueeze(0).expand(token_prefix.shape[0], -1, -1)
        prompt_vectors = torch.cat(
            [
                token_prefix,  # (dim0, 1, dim)
                (vas.cuda() - ctx).detach()+ctx,  # (dim0, n_ctx, dim)
