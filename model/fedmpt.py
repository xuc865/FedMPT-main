...
class CoOpPromptLearner(nn.Module):
    def __init__(self, cfg, dtype):
        ...
        n_ctx, ctx_depth = cfg.TRAINER.ML.CLS, cfg.MODEL.D_CTX
        ...
        ctx_vectors = torch.empty(n_attr_ctx, 512)
        nn.init.normal_(ctx_vectors, std=0.02)
        self.attr_ctx = nn.Parameter(ctx_vectors)  # to be optimized
...
class FedMPTCLIP(nn.Module):
    ...
    def set_prompt_prefix(self): 
        ...
        self.attn_text = ["color","background","scene type","spatial layout","pose"]
    ...
    def forward(self, image,classnames=None, dataname=None, req=False): 
        ...
        #     text_features_ = self.clip_model_.encode_condition_text(prompts_)
        ...
        # reshape to template * class:
        output = output.reshape(output.shape[0], 5, -1, output.shape[-1])
        # we get this. then we fuse the conditions.
        temp = self.cfg.TRAINER.ML.TEMP
        w = F.softmax(output/temp, dim=-1) 
        output1 = 5 * (output * w).sum(-1)  
        output2 = (output1 - output1.mean(dim=-1, keepdim=True)) / (output1.std(dim=-1, keepdim=True)+1e-5)
        logits = (output2 * F.softmax(output1.std(dim=-1, keepdim=True)/temp,dim=1)).mean(dim=1) # 32,5,20 
    ...
    def encode_condition_text(self, classnames): 
        prompts = [[self.prefix1 + " " + attn_text + " " + self.prompt_prefix + " " + name + \
            "." for name in classnames] for attn_text in self.attn_text]  
        tokenized_prompts = torch.cat([clip.tokenize(p) for p in prompts]) 
        with torch.no_grad():
            embedding = self.token_embedding(tokenized_prompts.to(self.device)).type(self.dtype)
        CONDS = self.cfg.TRAINER.ML.COND
        token_prefix = embedding[:, :1, :] # fixed indicts
        token_midfix = embedding[:, 1+CONDS:1+CONDS+1, :]  # blank 
        token_suffix = embedding[:, 1+CONDS+1+self.n_ctx:, :]   
        
        ctx ,attr_ctx = self.prompt_learner()
        ctx = ctx.to(self.dtype)
        ctx = ctx.unsqueeze(0).expand(token_prefix.shape[0], -1, -1)
        attr_ctx = attr_ctx.to(self.dtype)
        attr_ctx = attr_ctx.unsqueeze(0).expand(token_prefix.shape[0], -1, -1)
