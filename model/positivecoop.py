...
class MLCPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        tokenized_prompts_pos = torch.cat(tokenized_prompts_pos).cuda()
        with torch.no_grad():
            embedding_pos = clip_model.token_embedding(tokenized_prompts_pos).type(dtype)

        self.register_buffer("token_prefix_pos", embedding_pos[:, :1, :] ) # bot
        self.register_buffer("token_suffix_pos", embedding_pos[:, 1 + n_ctx_pos:, :]) # class + eot
...
class PositiveCoop(nn.Module):
    ...
    def forward(self, image, cls_id=None):
        ...
        output = 20 * F.conv1d(image_features_norm, text_features[:, :, None].to(image_features_norm.dtype))
        b, c, _ = output.shape
