...
def build_model_conv_proj(state_dict: dict, cfg):
    ...
    model = CLIP_conv_proj(
        embed_dim,
        224, vision_layers, vision_width, vision_patch_size,
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers
    )
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                ...
                w_spatial = state_dict[k][1:].T.reshape(1, -1, grid_size, grid_size)
                new_size = int((old_state_dict[k].shape[0] - 1) ** 0.5)
                w_spatial = F.interpolate(w_spatial, size=(new_size, new_size), mode='bicubic')
                w_spatial = w_spatial.reshape(-1, new_size**2).T
                new_w = torch.cat([state_dict[k][:1], w_spatial], dim=0)
