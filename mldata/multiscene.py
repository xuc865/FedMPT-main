...
from mldata.cls_to_names import multiscene_classes
from PIL import Image
import pdb

class MultiScene(ClusterMLDataset, DatasetBase):
    NAME = "MultiScene"
    def __init__(self, set_id, cfg, available_classes, traintest, clip_model): 
        ClusterMLDataset.__init__(self, cfg)
        self.path_dataset = os.path.join(cfg.DATASET.ROOT, 'MultiScene-Clean') 
        ...
        if traintest == "train":
            ...
            sets = "Test"
        file_csv = os.path.join(self.path_dataset, sets+'.csv')
        data_list = []
        only_gt = False
        header = True
        self.class_names = multiscene_classes 
    ...
    def __getitem__(self, index):
        ...
        path = os.path.join(self.path_dataset, 'images', path+'.jpg')
